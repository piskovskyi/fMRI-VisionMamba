...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.classifierLeft = nn.Sequential(OrderedDict(
            norm=norm_layer(self.num_features), # B,H,W,C
            permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
            avgpool=nn.AdaptiveAvgPool2d(1),
            flatten=nn.Flatten(1),
            head=nn.Linear(self.num_features, num_lh),
        ))

        self.classifierRight = nn.Sequential(OrderedDict(
            norm=norm_layer(self.num_features), # B,H,W,C
            permute=(Permute(0, 3, 1, 2) if not self.channel_first else nn.Identity()),
            avgpool=nn.AdaptiveAvgPool2d(1),
            flatten=nn.Flatten(1),
            head=nn.Linear(self.num_features, num_rh),
        ))
    ...
    def forward(self, x: torch.Tensor):
        #print(f"BEFORE PATCH EMBED: ", x.shape)
        ...
        #x = self.classifier(x)
        xL = self.classifierLeft(x)
        xR = self.classifierRight(x)
        return xL, xR
